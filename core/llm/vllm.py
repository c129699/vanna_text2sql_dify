"""
vLLM LLM 后端实现
vLLM 使用 OpenAI 兼容的 API，但可以独立配置
"""
...
if HAS_OPENAI:
    ...
    class VLLMBackend(BaseLLMBackend, OpenAI_Chat):
        """使用 vLLM 的 LLM 后端（基于 OpenAI 兼容 API）"""
        ...
        def __init__(
            ...
            Args:
                client: OpenAI 客户端实例（连接到 vLLM 服务）
                model: 模型名称
                config: 配置字典，包含：
                    - api_key: API 密钥（vLLM 通常不需要，可设置为 "EMPTY"）
                    - api_base: vLLM 服务的 OpenAI 兼容接口地址
        ...
        @classmethod
        def create(cls, config: Dict[str, Any]) -> 'VLLMBackend':
            """
            创建 vLLM LLM 后端实例
            ...
            print(f"✓ 使用 vLLM: {model_name} @ {api_base}")
            return instance
else:
    # 占位类
    class VLLMBackend:
